...
router.get("/route/list", (req, res) => {
  ...
  let sql =
    "SELECT r.*,m.`m_sid`,m.`m_photo`,m.`m_name` FROM `route`AS r INNER JOIN `member` AS m ON r.`m_sid` = m.`m_sid` ORDER BY r.`r_sid` " +
    req.query.orderby +
    " LIMIT " +
    page +
    " , 15";
...
router.get("/route/:sid", (req, res) => {
  const output = {};
  let sql =
    "SELECT r.*, m.`m_photo`,m.`m_name`  FROM `route` AS r LEFT JOIN `member` AS m ON r.`m_sid` = m.`m_sid` WHERE r_sid = ?";
  let sql2 =
        ...
        if (img_name !== "") {
          fs.createReadStream(req.file.path).pipe(
            fs.createWriteStream(
              __dirname + "/../public/r_upload_img/" + img_name
            )
          );
        }
        output.success = true;
...
router.post("/route/location", upload.none(), (req, res) => {
  ...
  let sql =
    "INSERT INTO`route_location`(`l_name`, `l_intro`,`r_sid`,`l_country`,`l_area`,`l_order`) VALUES " +
    question;
...
router.put(
  ...
  (req, res) => {
    ...
    let sql =
      "INSERT INTO `route_location` (`l_sid`,`l_name`,`r_sid`,`l_country`,`l_area`,`l_intro`,`l_order`) VALUES" +
      question +
      " ON DUPLICATE KEY UPDATE l_sid=VALUES(l_sid), `l_name`=VALUES(`l_name`), `r_sid`=VALUES(`r_sid`),`l_country`=VALUES(`l_country`),`l_area`=VALUES(`l_area`),`l_intro`= VALUES(`l_intro`),`l_order`=VALUES(`l_order`);";
